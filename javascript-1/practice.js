...
//Code here
function lovesCodeChecker(param) {
	if (param === true) {
		return true;
	}
	return false
...
const me = {
	...
	age: 26,
	greeter:()=> {
		return `Hello! my name is ${firstName} and i am from ${state}`
	}
};
...
// Code Here
let filtered = myNumbers.filter((arr => arr > 100))
...
const total = myNumbers.reduce(reducer, 0)
...
// Code Here
let myNumbersIndex = []
mynumbersIndex = myNumbers.forEach((arr, i, el)=> {
...
const forTheLoveOfGeorge = notGeorge.map((arr,i,el)=> {
	return "George";
})
...
//////////////////PROBLEM 20////////////////////
let enemies = people.filter(arr=> arr.friend == false );
...
// Code Here
const totallyAwesome = people.reduce((acc, el, i, arr) => {
